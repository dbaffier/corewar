...
/*   Updated: 2019/09/21 18:37:28 by bmellon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_params_len(t_param *params, int nbparam, char type, char opcode)
{
...
void	get_param_data(t_param *params, int nbparam, char *data, int pc)
{
	...
	while (i < nbparam)
	{
		...
		while (take < params[i]->param_size)
			params[i]->param_data = data[pc + 1 + params[i - 1]->param_size + 
				take++];
		params[i]->param_size
		i++;
	}
